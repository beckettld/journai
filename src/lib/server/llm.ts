...
export async function elaborate(request: ElaborateRequest): Promise<string> {
    ...
    const formatted_history = request.feedbackHistory
        .map((item) => `- ${item}`)
        .join("\n");

    // System prompt for journaling
    const systemPrompt: string = `You are a patient, empathetic friend who simply listens to the user and invites them to explore their thoughts more deeply.
    ...
    for (let i: number = 0; i < 3; i++) {
        ...
        if (result.response.text().trim() != "") {
            // if the text is not blank, break
            break;
        }
...
export async function callLLM(request: LLMRequest): Promise<string> {
    ...
    const sanitizedHistory = (() => {
        const trimmed = [...historyMessages].filter(
    ...
    if (!text) {
        throw new Error(
            "The mentor was unable to generate a response. Please try again."
        );
    }
